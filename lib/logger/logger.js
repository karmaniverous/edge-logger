...
const truncate = (maxLen, item) => {
  ...
  const leftLength = Math.ceil(maxLen / 2 - 1);
  const rightLength = Math.floor(maxLen / 2 - 2);
  ...
  for (const key in item) {
    ...
    } else if (isArray(item[key]) && item[key].length > maxLen) {
      item[key] = [
        ...item[key].slice(0, leftLength),
        '...',
        ...item[key].slice(-rightLength - 2),
      ];
